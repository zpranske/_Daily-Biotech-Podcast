...
Note: The cost per run is ~40 cents as of July 2026. TTS is the bulk of this cost (using gpt-4o-mini-tts at $12/1M tokens). Cost can be reduced significantly by delivering a text-only script.
...
* **Change the Podcast Persona:** Locate **neuro_prompt** inside the **prompts** folder and rewrite the instructions to do what you want it to do.
* **Change the Voice:** Look for 'voice="alloy"' within **config.py**. See available voices here: https://developers.openai.com/api/docs/guides/text-to-speech/. You can also custom prompt it to tweak pacing and tone by changing the prompt in **tts_instructions.txt** within the **prompts** folder –– this is best tested using openai.fm first.
